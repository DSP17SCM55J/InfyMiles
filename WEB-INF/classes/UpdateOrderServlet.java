...
public class UpdateOrderServlet extends HttpServlet {
    ...
    */
    ...
    throws ServletException, java.io.IOException {
		...
		showPage(response, "Welcome   ",(String) session.getAttribute("username"), request.getParameter("id"), request.getParameter("conf"));
		else 
		showPage(response, "Welcome  to Smart Portables", null,null,null); 
    }     
	
    protected void showPage(HttpServletResponse response, String message, String uname, String prodid, String ordconf)
    throws ServletException, java.io.IOException {
...
String sqlupd = "Update orders set Status='P' where username = ? and pid = ? and orderconf = ?";
PreparedStatement stupd = con.prepareStatement(sqlupd);
PreparedStatement st = con.prepareStatement(sql);
st.setString(1,user);
stupd.setString(1,user);
stupd.setString(2,prodid);
stupd.setString(3,ordconf);
stupd.executeUpdate();
...
while(rs.next()){
out.println("  <tr>");
out.println("<td><center><font size=\"3\"><b><p>" + rs.getString("orderconf") + "</p></b></font></center></td>");
...
out.println("<td><center><input type=\"button\" id=\"cartbutton" + rs.getString("pid") + "\" name=\"cartbutton" + rs.getString("pid") + "\" value=\"Process Order\" onclick=\"window.location.href='UpdateOrderServlet?id=" + rs.getString("pid")+ "&conf=" + rs.getString("orderconf") + "'\"/></center></td></tr>");				     


//out.println("<td><center><input type=\"button\" id=\"cartbutton" + rs.getString("pid") + "\" name=\"cartbutton" + rs.getString("pid") + "\" value=\"Cancel Order\" onclick=\"window.location.href='CancelOrderServlet?id=" + rs.getString("pid") + "'\"/></center></td></tr>");				     
}
